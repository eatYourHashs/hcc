...
execute if entity @s[tag=!hcc.diving,tag=!hcc.attacking,tag=!hcc.rising] if entity @s[tag=hcc.tamed] at @p[tag=hcc.tamer] positioned over world_surface positioned ~ ~30 ~ unless entity @s[distance=..40] facing ~ ~ ~ positioned as @s run tp @s ~ ~ ~ ~ ~
